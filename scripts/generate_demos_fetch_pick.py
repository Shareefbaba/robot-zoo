...
recorder = EpisodeRecorder(
    output_dir="/home/shareef/robot_zoo/data/demos/fetch_pick_cube",
    task="pick_cube",
    robot="fetch"
)
...
print(f"Collecting {args_cli.num_demos} Fetch pick-cube demonstrations.")
successful_eps = 0
...
for ep in range(args_cli.num_demos):
    ...
    is_success = final_z > 0.08

    if is_success:
        successful_eps += 1
        recorder.save_episode(success=True)
        print(f"  SUCCESS | z={final_z:.3f}")
    else:
        recorder.discard_episode()
        print(f"  FAILED  | z={final_z:.3f}")
