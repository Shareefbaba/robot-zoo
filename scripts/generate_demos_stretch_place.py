...
recorder = EpisodeRecorder(
    output_dir="/home/shareef/robot_zoo/data/demos/stretch_place", 
    task="pick_cube", 
    robot="stretch"
)
...
print(f"Collecting {args_cli.num_demos} Stretch place demonstrations.")
successful_eps = 0
...
while recorder._ep_idx < args_cli.num_demos:
    ...
    if pick_z > 0.4:
        # Phase 5: rotate wrist to move arm sideways
        jt[0,2]  = 0.4   # keep lift high
        jt[0,9]  = -0.8  # rotate wrist to shift position
        jt[0,10] = -0.6  # keep gripper closed
        jt[0,11] = -0.6
        for _ in range(80):
            stretch.set_joint_position_target(jt)
            stretch.write_data_to_sim()
        ...
        jt[0,2]  = 0.0   # lower lift
        jt[0,10] = -0.6  # keep closed
        jt[0,11] = -0.6
        for _ in range(100):
            stretch.set_joint_position_target(jt)
            stretch.write_data_to_sim()
    ...
    is_success = pick_z > 0.4 and final_z < 0.1
