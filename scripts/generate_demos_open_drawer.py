...
def main():
    ...
    recorder = EpisodeRecorder(
        output_dir="/home/shareef/robot_zoo/data/demos/franka_open_drawer",
        task="open_drawer",
        robot="franka",
    )
    ...
    print(f"Collecting {num_demos} open-drawer demonstrations.")
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            # cabinet_joint_pos is obs[18] — guaranteed correct joint
            obs_policy = env.unwrapped.observation_manager.compute_group("policy")
            drawer_opening = float(obs_policy[0, 18])

            robot = env.unwrapped.scene["robot"]
            joint_pos = robot.data.joint_pos[0]
            joint_vel = robot.data.joint_vel[0]

            if drawer_opening > SUCCESS_THRESHOLD and not episode_success:
                episode_success = True

            recorder.record_step(
                joint_pos = joint_pos,
                joint_vel = joint_vel,
                cube_pos  = cabinet_position[0],
                action    = actions[0],
                reward    = float(drawer_opening),
            )
            ...
            if dones.any():
                attempted += 1
                if episode_success and collected < num_demos:
                    recorder.save_episode(success=True)
                    collected += 1
                    print(f"  demo {collected:3d}/{num_demos} | drawer: {drawer_opening:.3f}m | attempts: {attempted}")
                else:
                    recorder.discard_episode()
                    print(f"  failed (attempt {attempted}) | drawer: {drawer_opening:.3f}m")
    ...
    print(f"Dataset complete: {collected} demos | success rate: {100*success_rate:.1f}%")
    env.close()
